...
14. 新手首轮引导优化（任务条、下一步引导、快捷开场句）
15. 安装入口可恢复（取消安装后仍可点击）
16. 安卓 Edge 安装兼容引导（手动菜单安装路径）
17. PWA 图标升级（`any + maskable + favicon`）
18. SW 更新策略补强（版本滚动 + 启动时主动更新检查）
...
3. P1: 安装转化率优化（浏览器分流文案 + 用户行为埋点）
4. P2: 微信小程序/原生 App 保持预研，不进入实现
