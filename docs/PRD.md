...
5. PWA 能力（安装入口、更新提示、离线状态提示）
6. 历史会话离线快照（最近会话可离线回看）
7. 新手引导能力（首轮任务条、下一步导航、开场句快捷填充）
...
1. 页面可访问不代表验收通过，必须同时满足“功能链路可用”
2. 登录成功后，不应因错误路由或缓存导致误报“离线”
3. 发布后 1 次刷新内应拿到新版本前端逻辑（不允许长期命中旧脚本）
4. 业务接口必须提供可验证健康入口（例如 `/health-backend`）

### 6.7 练习优先首屏要求（新增）

1. 首屏 3 秒内只强调一件事: 开始练习
2. 主视觉仅保留:
   - 场景/对象输入（可选模板）
   - 发言输入框（高优先级）
   - 主操作按钮（发送消息并获取反馈）
3. 登录/自检/引导/历史/复盘默认折叠或进入二级入口
4. 未登录时允许先输入，点击发送再触发登录拦截
5. 登录成功后自动继续刚才动作，避免重复输入
6. 错误处理应“就地可恢复”:
   - 明确错误分类（网络/鉴权/代理/服务异常）
   - 每类错误都提供下一步操作
   - 自检入口仅在失败态推荐出现

验收:
1. 新用户进入页面后 10 秒内可完成“输入一句话并点击发送”
2. 首轮流程中，非核心模块不会抢占用户主要注意力
3. 登录拦截后返回练习流程时，输入内容与意图保持完整
